...
exports.getSubmission = async (req, res) => {
   try {
      ...
      const getSubmission = await submissions.findOne({
         where: {
...
exports.evaluateTest = async (req, res) => {
   try {
      ...
      let test_score = score1 + score2 + score3 + score4;

      // Get Submission to Evaluate
      const getSubmission = await submissions.findOne({
         where: {
            id: req.params.id_submission,
         },
      });
      getSubmission.score1 = parseInt(score1);
      getSubmission.score2 = parseInt(score2);
      getSubmission.score3 = parseInt(score3);
      getSubmission.score4 = parseInt(score4);
      getSubmission.test_result = parseInt(test_score);

      await getSubmission.save();
      return res.status(201).send('Correcao Submetida com Sucesso');
   } catch (err) {
      return res.status(400).send({ error: err });
   }
};
